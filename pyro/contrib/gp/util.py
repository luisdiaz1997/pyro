...
def mggp_conditional(
    ...
    if Lff is None:
        Kff = kernel(X, groupsX).contiguous()
        Kff.view(-1)[:: N + 1] += jitter  # add jitter to diagonal
        Lff = torch.linalg.cholesky(Kff)
    Kfs = kernel(X, groupsX, Xnew, groupsXnew)
    ...
    if full_cov:
        Kss = kernel(Xnew, groupsXnew)
        Qss = W.matmul(W.t())
        cov = Kss - Qss
    else:
        Kssdiag = kernel(Xnew, groupsXnew, diag=True)
